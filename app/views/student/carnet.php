...
$stmt = $conexion->prepare(
    "SELECT u.correo, p.nombre, p.segundo_nombre, p.apellido, p.segundo_apellido, p.cedula, p.tipo_documento, c.nombre_carrera, p.foto_perfil, p.fecha_carnetizacion
    FROM usuario u
...
$correo    = $datos['correo'];
$carrera   = strtoupper($datos['nombre_carrera'] ?? 'ING. DE SISTEMAS');

$stmtSec = $conexion->prepare(
    "SELECT s.nombre_seccion FROM solicitud_inscripcion si
    JOIN seccion s ON si.id_seccion = s.id
    WHERE si.id_estudiante = :id AND si.estado = 'Aceptada'
    ORDER BY si.id DESC LIMIT 1"
);
$stmtSec->execute([':id' => $id_usuario]);
$secRow   = $stmtSec->fetch(PDO::FETCH_ASSOC);
$has_enrollment = (bool)$secRow;
$seccion  = $has_enrollment ? strtoupper($secRow['nombre_seccion']) : 'SIN SECCION';

$vence = 'DIC ' . (date('Y') + 1);

$qrPayload = "Nombre: {$nombre} | Cedula: {$cedula} | Correo: {$correo}";

$pdfUrl = URLROOT . '/carnet/CARNET_UNEFA_Plantilla estudiantes.pdf';
...
} elseif (!$has_enrollment) {
    $error_carnet = "No disponible: Aun no estas inscrito en ninguna seccion aprobada. Debes inscribirte primero para obtener tu carnet.";
} elseif ($fecha_c) {
...
<div class="container py-4 carnet-page">
    ...
    <div class="text-center mb-4 d-print-none">
        <h2 class="fw-black" style="color:#003366;">Carnet Digital UNEFA</h2>
    ...
    <div class="d-flex justify-content-center gap-3 mt-3 flex-wrap">
            ...
            <a href="index.php?page=mi_qr" class="btn btn-outline-primary rounded-pill px-4">
                <i class="fas fa-qrcode me-2"></i>Ver mi QR
            </a>
...
async function drawFrontOverlay(pdfW, pdfH) {
    ...
    ctx.fillText(`${CARNET_DATA.section}  ·  ${CARNET_DATA.idNumber}`, cx, ty + 56 * SCALE);
